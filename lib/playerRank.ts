...
async function fetchAllPlayersAuraInfo(): Promise<PlayerAuraInfo[]> {
  ...
  try {
    ...
    if (!Array.isArray(data)) {
      if (data && typeof data === 'object' && data.ci) { // data is a single player object
        data = [data];
      } else if (data && typeof data === 'object' && data.data && typeof data.data === 'object' && data.data.ci) { // data.data is a single player object
        data = [data.data];
      } else {
