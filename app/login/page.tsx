...
export default function LoginPage() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (failedAttempts >= 3) {
      ...
      return <div>Demasiados intentos fallidos. Por favor, contacta a un administrador.</div>;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (failedAttempts + 1 >= 3) { // Check if this attempt makes it 3 or more
  ...
  return (
    <div className="flex items-center justify-center min-h-[calc(90vh-64px)]"> {/* Adjust min-h to account for header height */}
      <Card className="w-full max-w-md bg-[#2A2A3E]">
        ...
        <CardFooter className="flex flex-col gap-2"> {/* Moved CardFooter outside form */}
